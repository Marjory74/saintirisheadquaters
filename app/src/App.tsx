...
import { Home as HomeIcon, Search, Clock3, Images, Heart, Shield, LogIn } from 'lucide-react'
      ...
      <div className="mx-auto flex max-w-6xl items-center gap-2 px-4 py-2.5">
        ...
        <div className="ml-auto flex items-center gap-1.5 md:gap-2">
          ...
          <button
            ...
            data-cursor
            title={user ? 'หลังบ้าน' : 'เข้าสู่ระบบ'}
            className={`flex items-center gap-1.5 rounded-full border-2 px-3 py-1.5 text-xs font-bold transition-all md:text-sm ${
              pathname === '/admin' || pathname === '/login'
                ? 'border-[#54453f] bg-[#e9e4f5] text-[#54453f] shadow-[2px_2px_0_0_#54453f]'
                : 'border-transparent text-[#7b6b9e] hover:border-[#e9e4f5] hover:bg-white'
            }`}
          >
            {user ? <Shield className="h-3.5 w-3.5 md:h-4 md:w-4" /> : <LogIn className="h-3.5 w-3.5 md:h-4 md:w-4" />}
            <span className="hidden sm:inline">{user ? 'หลังบ้าน' : 'ล็อกอิน'}</span>
          </button>
        </div>
      </div>
    </nav>
    ...
    <div className="min-h-screen">
      ...
      <footer className="relative z-10 border-t-2 border-[#ffd3e0] bg-[#ffe6ef] px-6 py-8 text-center">
        ...
        <p className="mt-1 flex items-center justify-center gap-1.5 text-sm text-[#54453f]/80">
          ...
          <Heart className="h-3.5 w-3.5 fill-[#f78fb8] text-[#f78fb8]" /> {profile.badgeId}
        </p>
