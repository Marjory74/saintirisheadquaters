...
export type Page = 'home' | 'detective' | 'timeline' | 'gallery'

const NAV: { id: Page; label: string; icon: typeof HomeIcon }[] = [
  { id: 'home', label: 'หน้าแรก', icon: HomeIcon },
  { id: 'detective', label: 'แฟ้มนักสืบ', icon: Search },
  { id: 'timeline', label: 'ไทม์ไลน์', icon: Clock3 },
  { id: 'gallery', label: 'แกลเลอรี', icon: Images },
]
    ...
    <div className="min-h-screen">
      ...
      <nav className="sticky top-0 z-[70] border-b-2 border-[#ffd3e0] bg-[#fff9fb]/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center gap-2 px-4 py-2.5">
          <button onClick={() => go('home')} data-cursor className="mr-2 flex items-center gap-2">
      ...
      {/* pages */}
      <main key={page} className="page-enter relative z-10">
        {page === 'home' && <Home go={go} />}
        {page === 'detective' && <Character />}
      ...
      <footer className="relative z-10 border-t-2 border-[#ffd3e0] bg-[#ffe6ef] px-6 py-8 text-center">
        ...
        <p className="mt-1 flex items-center justify-center gap-1.5 text-sm text-[#54453f]/80">
          สำนักงานใหญ่เซนต์ไอริส <Heart className="h-3.5 w-3.5 fill-[#f78fb8] text-[#f78fb8]" />
          {' '}
          {profile.badgeId}
        </p>
